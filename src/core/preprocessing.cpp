...
void InferenceBuffer::ensureCapacity(int height, int width, int channels) {
...
void letterBox(const cv::Mat &image, cv::Mat &outImage,
               const cv::Size &newShape, const cv::Scalar &color, bool autoSize,
...
void letterBoxCentered(const cv::Mat &image, cv::Mat &outImage,
                       const cv::Size &newShape, bool autoSize, bool scaleFill,
                       bool scaleUp, bool center, int stride,
                       const cv::Scalar &paddingValue, int interpolation) {
...
BoundingBox scaleCoords(const cv::Size &letterboxShape,
                        ...
                        const cv::Size &originalShape, bool clip) {

  float gain =
      std::min(static_cast<float>(letterboxShape.height) / originalShape.height,
               static_cast<float>(letterboxShape.width) / originalShape.width);

  int padX = static_cast<int>(
      std::round((letterboxShape.width - originalShape.width * gain) / 2.0f));
  int padY = static_cast<int>(
      std::round((letterboxShape.height - originalShape.height * gain) / 2.0f));
  ...
  if (clip) {
    ...
    result.width =
        utils::clamp(result.width, 0, originalShape.width - result.x);
    result.height =
        utils::clamp(result.height, 0, originalShape.height - result.y);
  }
...
KeyPoint scaleKeypoint(const cv::Size &letterboxShape, const KeyPoint &keypoint,
  ...
  if (clip) {
    result.x = utils::clamp(result.x, 0.0f,
                            static_cast<float>(originalShape.width - 1));
    result.y = utils::clamp(result.y, 0.0f,
                            static_cast<float>(originalShape.height - 1));
  }
...
void letterBoxToBlob(const cv::Mat &image, InferenceBuffer &buffer,
  ...
  std::fill(buffer.blob.begin(), buffer.blob.begin() + dstH * dstW * 3,
            padNorm);

  // Resize if needed
  if (newW != srcW || newH != srcH) {
    cv::resize(image, buffer.resized, cv::Size(newW, newH), 0, 0,
               cv::INTER_LINEAR);
  } else {
