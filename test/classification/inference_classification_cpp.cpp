...
struct ResultsCls {
  ...
  std::string task;
  std::unordered_map<std::string, std::vector<SingleInferenceResultCls>>
      inferenceResults;
};
...
void findModels(const std::string &modelsDir,
                std::vector<std::string> &modelFiles) {
  if (!fs::exists(modelsDir) || !fs::is_directory(modelsDir)) {
    std::cerr << "Error: Models path does not exist or is not a directory: "
              << modelsDir << std::endl;
    return;
  }
  ...
  if (modelFiles.empty()) {
    std::cerr << "Warning: No .onnx models found in directory: " << modelsDir
              << std::endl;
  }
}
...
void fromMapToJson(const std::unordered_map<std::string, ResultsCls> &results,
  ...
  for (const auto &[modelName, resultsForModel] : results) {
    ...
    outputJson[modelName]["weights_path"] =
        resultsForModel.weightsPath.substr(basePath.length());
    outputJson[modelName]["task"] = resultsForModel.task;
    outputJson[modelName]["results"] = nlohmann::json::array();

    for (const auto &[imagePath, inferenceResVec] :
         resultsForModel.inferenceResults) {
...
int main(int argc, char *argv[]) {
  ...
  for (const auto &modelFullPath : modelFiles) {
    ...
    std::cout << "\n ######## Running inference for model: " << modelFullPath
              << " ########" << std::endl;
    runInference(modelFullPath, labelsPath, imageFiles, isGPU,
                 allResults[modelName].inferenceResults);
