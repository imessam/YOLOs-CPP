...
// Using standardized KeyPoint from YOLO-Utils.hpp
...
void runInference(
  ...
  for (const auto &imagePath : imageFiles) {
    ...
    for (const auto &result : results) {
      std::cout << "Pose: Class=" << result.classId
...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::string> models = {"yolov8n-pose", "yolo11n-pose"};
