...
void fromMapToJson(const std::unordered_map<std::string, Results> &results,
  ...
  for (const auto &[modelName, results] : results) {
    ...
    outputJson[modelName]["weights_path"] =
        results.weightsPath.substr(basePath.length());
...
int main(int argc, char *argv[]) {
  ...
  std::string basePath =
      XSTRING(BASE_PATH_DETECTION); // Base path defined in CMakeLists.txt
  ...
  std::vector<std::string> models = {
      "YOLOv5nu_voc", "YOLOv6n_voc",  "YOLOv8n_voc", "YOLOv9t_voc",
      "YOLOv10n_voc", "YOLOv11n_voc", "YOLOv12n_voc"};
  ...
  for (const auto &model : models) {
    ...
    runInference(modelPath, labelsPath, imageFiles, inferenceConfig, isGPU,
                 allResults[model].inferenceResults);
