...
struct Results {
  ...
  std::string task;
  std::unordered_map<std::string, std::vector<SingleOBBInferenceResult>>
      inferenceResults;
};
...
void runInference(
  ...
  for (const auto &imagePath : imageFiles) {
    ...
    std::vector<OBB_Detection> results =
        detector.detect(image, confThreshold, iouThreshold);
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::high_resolution_clock::now() - start);

    std::cout << "OBB Detection completed in: " << duration.count() << " ms"
              << std::endl;
    std::cout << "Number of OBB detections found: " << results.size()
              << std::endl;

    for (size_t i = 0; i < results.size(); ++i) {
      std::cout << "OBB Detection " << i << ": Class=" << results[i].classId
                << ", Confidence=" << results[i].conf << ", Center=("
                << results[i].box.x << "," << results[i].box.y << ")"
                << ", Size=(" << results[i].box.width << "x"
                << results[i].box.height << ")"
      ...
      singleResult.normalized_cx =
          results[i].box.x / static_cast<float>(image_width);
      singleResult.normalized_cy =
